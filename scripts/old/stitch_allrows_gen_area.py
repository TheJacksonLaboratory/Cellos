...
from skimage.morphology import label, remove_small_objects,square,binary_closing,dilation
...
def remove_big_square(image_2um_r15_labeled_blobs):
    segments = image_2um_r15_labeled_blobs.max(axis=2)
    image_2um_r15_labeled_blobs_nbig = np.copy(segments)
    component_sizes = np.bincount(segments.ravel())
    too_big = component_sizes > 100000
    too_big_mask = too_big[segments]
    image_2um_r15_labeled_blobs_nbig[too_big_mask] = 0

    return image_2um_r15_labeled_blobs_nbig
...
image_2um_r15_labeled_blobs_nbig = remove_big_square(labeled_blobs_2um_big)

image_2um_r15_labeled_blobs_nbig_dilated_300 = dilation (image_2um_r15_labeled_blobs_nbig, square(300))
image_2um_r15_labeled_blobs_nbig_dilated_closed_200 = binary_closing(image_2um_r15_labeled_blobs_nbig_dilated_300,
                                                                     selem=square(200), out=None)
pixels = cv2.countNonZero(image_2um_r15_labeled_blobs_nbig_dilated_closed_200.astype('int'))
area = pixels*(6.45*6.45)

with open(output_dir+well_loc+'area.txt', 'w') as f:
    f.write('%d' % area)

plt.figure(figsize=(10,17))
plt.imshow(image_2um_r15_labeled_blobs_nbig_dilated_closed_200)
plt.savefig(output_dir+well_loc+'_organoids_area.pdf')
